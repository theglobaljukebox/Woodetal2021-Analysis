...
DATA_PATH = './data/downloaded_data.csv'
metadata = pd.read_csv('./data/conversion_metadata.csv')

with open('./data/code_mappings.json') as f:
...
def encode_data(line_num, data):
    ...
    description = ''
    shortname = ''
    column_name = "line_"+str(line_num)
...
def main():
    ...
    bar = Bar('Converting', max=(37))
    for i in range(37):
        bar.next()
        line_key = "line_"+str(i+1)
        lines[line_key] = encode_data(i+1, data)
    bar.finish()
    with open('./data/conversion_guide.json', 'w') as f:
        json.dump(lines, f)
